...
Confronté à l'impossibilité de réaliser un Projet Collaboratif Innovant (PCI) classique inter-stagiaires en tant qu'\textbf{unique stagiaire NSI de la région Normandie} (en raison du très faible nombre de postes ouverts au niveau national cette année-là), j'ai proposé à l'INSPÉ un projet innovant intra-établissement impliquant plusieurs disciplines.
...
\subsection{Conception de l'artefact didactique : un moteur de médiation pédagogique}

En tant qu'ingénieur de formation et concepteur de ressources, j'ai développé un \textbf{moteur de jeu sur mesure} en Python (\url{https://github.com/estebe2000/v3}). Loin d'une simple performance informatique, cette démarche de conception répondait avant tout à une exigence de \textbf{didactisation et de médiation socio-technique} \parencite{rabardel_hommes_technologies,chevallard_transposition} :

\subsubsection{Ingénierie de la ressource et découplage didactique}
\begin{itemize}[leftmargin=*]
    \item \textbf{Encapsulation de la complexité (Apprêtement didactique)} : Masquer la complexité algorithmique de bas niveau (gestion des boucles système, collisions physiques) pour permettre aux apprenants de se concentrer sur les concepts clés et la création.
    \item \textbf{Architecture modulaire et inter-disciplinaire} : Découpler strictement les couches applicatives (scénario/dialogues, ressources graphiques, logique algorithmique, environnement sonore) afin de permettre la contribution simultanée d'équipes aux profils et compétences disciplinaires très variés.
    \item \textbf{Médiation multilingue et écosystème d'apprentissage} : Intégration d'un système de traduction dynamique (français, anglais, italien) facilitant la médiation culturelle et linguistique.
\end{itemize}

Cette démarche s'inscrit pleinement dans les principes de l'ingénierie pédagogique et de formation \parencite{ingenierie_digiforma} : concevoir un environnement de formation qui soutient l'action réflexive et le travail collaboratif, en posant la question pivot : \enquote{Comment la ressource technique devient-elle un instrument d'apprentissage et de coopération ?} \parencite{rabardel_hommes_technologies,recherche_action_education}.
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Septembre - Octobre} : Conception du moteur de jeu et recrutement des équipes
    \item \textbf{Novembre - Décembre} : Premier sprint (définition du scénario, premiers graphismes)
    \item \textbf{Janvier - Février} : Deuxième sprint (développement des niveaux, musiques)
    \item \textbf{Mars - Avril} : Troisième sprint (traductions, tests, corrections)
    \item \textbf{Mai} : Finalisation et présentation aux classes participantes
\end{itemize}
...
\subsubsection{EC 163 / 263 (Animation du collectif) | RNCP BC09 (Communauté éducative)}
...
\subsubsection{EC 162 / 262 (Numérique éducatif) | RNCP BC05 (Didactique \& Numérique)}
...
\subsubsection{EC 124 / 224 (Ingénierie de formation) | RNCP BC04 (Conduite de projet)}

La démarche suivie (Analyse du problème $\rightarrow$ Conception d'une solution $\rightarrow$ Mise en œuvre $\rightarrow$ Analyse des résultats) est celle d'un praticien-chercheur \parencite{schon_reflective_practitioner,recherche_action_education}. Cette recherche-action illustre la conduite autonome d'un projet pédagogique complexe.
...
\subsection{Transition réflexive : des limites évaluatives du projet collectif vers l'ingénierie d'évaluation (Lien avec l'Expérience 3)}

Si ce Projet Collaboratif Innovant a démontré l'efficacité de la pédagogie par projet interdisciplinaire, il a également mis en lumière une limite majeure de l'ingénierie de formation traditionnelle : \textbf{la difficulté d'évaluer équitablement la contribution individuelle au sein d'un artefact collectif ambitieux}. 

Comment attribuer une note juste quand un élève s'est investi sur l'algorithmique complexe tandis qu'un autre a coordonné la traduction ou le graphisme, sans générer un sentiment de frustration ou d'injustice redistributive ? C'est précisément cette rupture réflexive née de l'Expérience 2 qui m'a motivé à engager une recherche-action ciblée sur l'\textbf{ingénierie de l'évaluation régulatrice} : la conception et l'expérimentation de la \textbf{Notation à l'Enveloppe (Chapitre 11 - Expérience 3)}.

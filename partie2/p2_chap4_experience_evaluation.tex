...
\subsection{Présentation de la méthode : « Notation à l'Enveloppe » ou « Notation à l'Italienne »}

Cette méthode d'évaluation différenciée, également appelée \textbf{« Notation à l'Enveloppe »} ou \textbf{« Notation à l'Italienne »}, constitue une réponse innovante aux défis de l'évaluation équitable du travail de groupe. Elle s'appuie sur une auto-régulation par les pairs sous supervision pédagogique, permettant une différenciation fine des contributions individuelles dans un projet collectif.

\subsection{Le point de départ : un problème d'équité récurrent}

Cette méthode est née d'un double constat professionnel \parencite{evaluation_pairs_utc,evaluation_pairs_univ_poitiers} :

\begin{enumerate}[leftmargin=*]
    \item \textbf{L'injustice perçue} : Dans la notation de groupe classique, tous les membres reçoivent la même note indépendamment de leur investissement réel, ce qui frustre les élèves impliqués et déresponsabilise les moins investis
    \item \textbf{L'impossibilité d'observation exhaustive} : En tant qu'enseignant, je ne peux pas observer en permanence et finement la contribution individuelle de chaque élève dans chaque groupe, surtout avec 25 élèves répartis en 6 groupes
\end{enumerate}

Ces constats rejoignent les problématiques identifiées dans la littérature sur l'évaluation \parencite{evaluation_formative}, qui souligne la difficulté d'évaluer équitablement le travail collectif.
...
Pour soutenir cette démarche, j'ai créé plusieurs documents qui sont aujourd'hui réutilisables et partageables. Ces outils constituent une véritable ingénierie de l'évaluation, affinée au fil des années et transférable à d'autres contextes.
...
\subsection{Transférabilité vers le référentiel MEEF et l'enseignement supérieur}

Cette expérience démontre des compétences d'expert, parfaitement alignées avec les attendus du Master MEEF PIF \parencite{rncp38152} :

\subsubsection{BC06 - Praticien réflexif et innovateur}

Je n'invente pas seulement des méthodes, j'en crée de nouvelles répondant à des enjeux réels \parencite{schon_reflective_practitioner,perrenoud_pratique_reflexive,recherche_action_education}.

\subsubsection{BC05 - Compétences en évaluation formative}

J'utilise l'évaluation comme un \textbf{levier d'apprentissage} pour développer l'autonomie, la responsabilité et les compétences sociales \parencite{evaluation_formative}.

\subsubsection{BC04 - Pertinence exceptionnelle pour l'IUT}

Cette méthode est une simulation directe des pratiques d'évaluation en entreprise \parencite{competences_transversales_eurecia}, exceptionnellement pertinente pour mes étudiants de BUT.
